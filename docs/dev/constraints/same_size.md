...
match in a complete puzzle. A free cell that would merge marked groups of the
same colour is pruned, including a free marked cell that would join an existing
marked group.

### apply

The largest currently relevant group supplies the target size. For each smaller
relevant group, `SameSize.apply` delegates to a temporary `GroupSize` constraint,
then retags the resulting move to the `SZ` instance. This reuses the existing
Group Size growth rules: forced exits, overshoot pruning, reachability
articulation, and impossible enclosed groups.
