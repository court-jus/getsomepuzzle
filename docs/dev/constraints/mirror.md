...
cell — like the other global constraints (`QA`, `FM`, `GC`, `SH`, `BB`). No
branch is needed in `to_flutter.dart` (that dispatcher renders in-cell glyphs
for `CellsCentricConstraint`s only).
