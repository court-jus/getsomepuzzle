...
class MirrorConstraint extends Constraint {
  ...
  int _countOf(Puzzle puzzle, int half) {
    var count = 0;
    for (int idx = 0; idx < puzzle.cells.length; idx++) {
      if (puzzle.cells[idx].value == color &&
  ...
  bool verify(Puzzle puzzle) {
    final live = puzzle.cells.where(
      (c) => c.value == CellValue.free && c.options.contains(color),
    );
    if (live.isNotEmpty) return true; // undecided — cannot judge yet
    return _countOf(puzzle, 0) == _countOf(puzzle, 1);
  }
  ...
  Move? apply(Puzzle puzzle) {
    ...
    if (count[0] == count[1]) {
      if (live[0].isEmpty && live[1].isNotEmpty) {
        return RemoveOption(live[1].first, color, this, complexity: 1);
      }
      if (live[1].isEmpty && live[0].isNotEmpty) {
        return RemoveOption(live[0].first, color, this, complexity: 1);
      }
    }
    return null;
  }
