...
class ShapeConstraint extends Motif {
  ...
  // Shape carries a single constrained colour (inferred from the motif),
  // not the per-cell motif palette — override Motif's motif-derived set.
  @override
  ...
  Move? apply(Puzzle puzzle) {
    ...
    for (final group in groups) {
      ...
      // A group can only still grow through a free neighbour that can still
      // become `color`. On a 3+ colour domain a free neighbour may have
      // `color` pruned from its options; such a cell freezes that border. If
      // every free neighbour is pruned, the group is effectively closed and
      // must already match a variant exactly (Level 1), otherwise we would
      // miss an impossibility on an undersized frozen group.
      final isOpen = freeNeighbors.any(
        (n) => puzzle.cells[n].options.contains(color),
      );
      ...
      if (group.length == shapeSize && _groupMatchesAVariant(group, puzzle)) {
        for (var freeNeighbor in freeNeighbors) {
          if (puzzle.cells[freeNeighbor].options.contains(color)) {
            return RemoveOption(freeNeighbor, color, this, complexity: 0);
          }
        }
      ...
      for (final neighbor in freeNeighbors) {
        if (!_groupCanFitInSomeVariant([...group, neighbor], puzzle) &&
          ...
          return RemoveOption(neighbor, color, this, complexity: 2);
        }
      }
      ...
      for (final neighbor in freeNeighbors) {
        if (!anyCompletion.contains(neighbor) &&
            puzzle.cells[neighbor].options.contains(color)) {
          return RemoveOption(neighbor, color, this, complexity: 4);
        }
      }
    }
  ...
  bool verify(Puzzle puzzle) {
    ...
    for (final group in groups) {
      ...
      // Does this group have a free neighbour that can still become `color`?
      // Only then can it still grow. A free neighbour with `color` pruned
      // (3+ colour domain) can never extend the group, so a group fenced in
      // by such cells is effectively closed and must match a variant exactly
      // — otherwise an undersized frozen group would wrongly pass as "still
      // growing" (too lenient).
      final bool isOpen = group.any(
        (idx) => puzzle
            ...
            .any(
              (n) =>
                  puzzle.cellValues[n] == CellValue.free &&
